...
public:
  ...
  : Node("odometry_publisher")
  {
    ...
    subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", rclcpp::SensorDataQoS(), std::bind(&PubOdomNode::rover_odom_callback, this, _1));

    // Initialize the transform broadcaster
    tf_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);

    // Parameters for tuning odometry and frame names (useful for SLAM)
    this->declare_parameter("odom_kvx", 1.0);
    this->declare_parameter("odom_kvy", 1.0);
    this->declare_parameter("odom_kth", 1.0);
  this->declare_parameter("odom_frame", std::string("odom"));
  this->declare_parameter("base_frame", std::string("base_footprint"));

    odom_kvx = this->get_parameter("odom_kvx").as_double();
    odom_kvy = this->get_parameter("odom_kvy").as_double();
    odom_kth = this->get_parameter("odom_kth").as_double();
